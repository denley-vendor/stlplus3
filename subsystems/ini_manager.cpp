...
namespace stlplus
{
	...
	////////////////////////////////////////////////////////////////////////////////
	// local utilities, copy from string_utilities.cpp
	
	static std::string trim_left(const std::string& val, char flag = ' ')
	{
	...
	static std::string trim_right(const std::string& val, char flag = ' ')
	{
		std::string result = val;
	...
	static std::string trim(const std::string& val, char flag = ' ')
	{
	...
	public:
		...
		void set_variable(const std::string& value) { m_text = m_name + " = " + value; m_value = value; }
	...
	public:
		ini_section(const std::string& title) : 
		  ...
		  bool set_variable(unsigned line, const std::string& variable, const std::string& value)
		  {
			  for (std::list<ini_entry>::iterator entry = m_entries.begin(); entry != m_entries.end(); entry++)
			  {
				  if (entry->variable_name() == variable)
				  {
					  entry->set_variable(value);
					  return true;
				  }
			  }
	  ...
	  bool read_file(const std::string& filename)
      {
		  ...
		  while(std::getline(source,line))
		  {
			  ...
			  else
			  {
				  ...
				  std::vector<std::string> nameValues = split(line, "=");
				  std::string name = trim(nameValues[0]);
				  std::string value = trim(nameValues[1]);
/*
				  std::string name;
				  while (i < line.size() && line[i] != '=')
					  name += line[i++];
...
*/				  section->add_variable(line_number, name, value);
	  ...
	  bool set_variable(const std::string& section, const std::string& variable, const std::string& value)
      {
		  if (!m_writable) return false;
		  std::list<ini_section>::iterator found = find_section(section);
		  if (found->set_variable(0,variable,value))
			  m_changed = true;
		  return true;
      }
  ...
  public:
	  ...
	  bool set_variable(const std::string& section, const std::string& variable, const std::string& value, unsigned depth)
      {
		  return m_files[depth].set_variable(section, variable, value);
      }
  ...
  bool ini_manager::set_variable(const std::string& section, const std::string& variable, const std::string& value, unsigned depth)
  {
	  return m_body->set_variable(section, variable, value, depth);
  }
  
  bool ini_manager::set_variable_ex( const std::string& section, const std::string& variable, const std::string& value, unsigned depth)
  {
	  if (variable_exists(section, variable, depth))
		  return set_variable(section, variable, value, depth);
  ...
  {
	  return add_variable(section, variable, join(values), depth);
  }
  
  bool ini_manager::set_variable( const std::string& section, const std::string& variable, const std::vector<std::string>& values, unsigned depth)
  {
	  return set_variable(section, variable, join(values), depth);
  }
    
  bool ini_manager::set_variable_ex( const std::string& section, const std::string& variable, const std::vector<std::string>& values, unsigned depth)
  {
	  return set_variable_ex(section, variable, join(values), depth);	  
  }
  ...
  std::string ini_manager::join(const std::vector<std::string>& values)
  {
